...
// Not needed as we have no vendor files now that the OAuth piece has been removed
// require_once APP . "AvailablePlugin" . DS . "RestTestDccProvisioner" . DS . "Vendor" . DS . "autoload.php";

class CoRestTestDccProvisionerTarget extends CoProvisionerPluginTarget {
  // Define class name for cake
  public $name = "CoRestTestDccProvisionerTarget";
  ...
  public $displayField = "resttestdcc_url";
   ...
   * @param  String $resttestdcc_url Base URL of the DCC Server
   ...
   */
   protected function dccConnect($target_data) {
    $resttestdcc_url = $target_data['resttestdcc_url'];
    $base_uri        = $resttestdcc_url . '/rest-dcc/';
  ...
  protected function dccConnectWithOauth($target_data) {
    $resttestdcc_url = $target_data['resttestdcc_url'];
    $provider = new \League\OAuth2\Client\Provider\GenericProvider([
        'clientId'                => $target_data['client_id'],
        'clientSecret'            => $target_data['client_secret'],
        'redirectUri'             => $target_data['callback_url'],
        'urlAuthorize'            => $resttestdcc_url . '/oauth/oauth2/auth',
        'urlAccessToken'          => $resttestdcc_url . '/oauth/oauth2/token',
        'urlResourceOwnerDetails' => $resttestdcc_url . '/oauth/userinfo'
    ]);
    ...
    if(!empty($access_token) && !empty($refresh_token)) {
      ...
      $target = new CoRestTestDccProvisionerTarget();
      ...
      if($target->saveField('access_token', $access_token) &&
        ...
        $this->Flash->set(_txt('er.resttestdccprovisioner.access_token'), array('key' => 'error'));
    }
  ...
  public function provision($coProvisioningTargetData, $op, $provisioningData) {
    ...
    if($syncGroupsForPerson || $syncMembersForGroup){
      $client = $this->dccConnect($coProvisioningTargetData['CoRestTestDccProvisionerTarget']);

      // Make a hash table of the groups in the DocDB database
      $response = $client->get('SecurityGroup');
      if($response->getStatusCode() != 200) {
        throw new RuntimeException(_txt('er.resttestdccprovisioner.securitygroup.query.failed'));
      }
      $securityGroup = array();
  ...
  protected function syncMembersForCoGroup($client, $security_group, $provisioningData) {
    ...
    if($response->getStatusCode() != 200) {
      throw new RuntimeException(_txt('er.resttestdccprovisioner.usersgroup.query.failed'));
    }
    $users_group = array();
     ...
     $usersgroup_data = array(
      ...
      if(!array_key_exists( $dccid, $users_group )) {
        ...
        if($response->getStatusCode() != 201){
          throw new RuntimeException(_txt('er.resttestdccprovisioner.usersgroup.create.failed') . ' (DccDocDbID = ' . $dccid . ')');
        } else {
          $this->CoProvisioningTarget->Co->CoPerson->HistoryRecord->record($coPersonId,
                                                                           ...
                                                                           _txt('pl.resttestdccprovisioner.added', array($co_group_name)));
        } 
      }
    }
    ...
    foreach(array_keys($users_group) as $u){
      if(!array_key_exists($u, $co_users_in_group)){
        ...
        if($response->getStatusCode() != 200){
          throw new RuntimeException(_txt('er.resttestdccprovisioner.usersgroup.delete.failed') . ' (UsersGroupID = ' . $users_group[$u] . ')');
        } else {
          $this->CoProvisioningTarget->Co->CoPerson->HistoryRecord->record($coPersonId,
                                                                           null,
                                                                           null,
                                                                           ...
                                                                           CakeSession::read('Auth.User.co_person_id'),
                                                                           ActionEnum::ProvisionerAction,
                                                                           _txt('pl.resttestdccprovisioner.removed', array($co_group_name)));
        }
      }
    }
  ...
  protected function syncGroupsForCoPerson($client, $security_group, $provisioningData, $activatePerson) {
    ...
    if($response->getStatusCode() != 200 ) {
      throw new RuntimeException(_txt('er.resttestdccprovisioner.institution.notfound'));
    }
    ...
    if ( $code == 200 ) {
      ...
      if($response->getStatusCode() != 200) {
        throw new RuntimeException(_txt('er.resttestdccprovisioner.author.update.failed') . ' (DccDocDbID = ' . $dccid . ') ' . $full_name);
      }

      $response = $client->put('EmailUser/' . $dccid, [GuzzleHttp\RequestOptions::JSON => $emailuser_data]);
      if($response->getStatusCode() != 200) {
        throw new RuntimeException(_txt('er.resttestdccprovisioner.emailuser.update.failed') . ' (DccDocDbID = ' . $dccid . ') ' . $full_name);
      }

      $response = $client->put('RemoteUser/' . $dccid, [GuzzleHttp\RequestOptions::JSON => $remoteuser_data]);
      if($response->getStatusCode() != 200) {
        throw new RuntimeException(_txt('er.resttestdccprovisioner.remoteuser.update.failed') . ' (DccDocDbID = ' . $dccid . ')' . $full_name);
      }
      ...
      if($response->getStatusCode() != 201) {
        throw new RuntimeException(_txt('er.resttestdccprovisioner.author.create.failed') . ' (DccDocDbID = ' . $dccid . ')' . $full_name);
      }

      $response = $client->post('EmailUser', [GuzzleHttp\RequestOptions::JSON => $emailuser_data]);
      if($response->getStatusCode() != 201) {
        throw new RuntimeException(_txt('er.resttestdccprovisioner.emailuser.create.failed') . ' (DccDocDbID = ' . $dccid . ')' . $full_name);
      }

      $response = $client->post('RemoteUser', [GuzzleHttp\RequestOptions::JSON => $remoteuser_data]);
      if($response->getStatusCode() != 201) {
        throw new RuntimeException(_txt('er.resttestdccprovisioner.remoteuser.create.failed') . ' (DccDocDbID = ' . $dccid . ')' . $full_name);
      }
    } else {
      // Something went wrong
      throw new RuntimeException(_txt('er.resttestdccprovisioner.author.query.failed') . '(DccDocDbID = ' . $dccid . ')' . $full_name );
    }

    // Make a hash table of the user's current DocDB groups
    $response = $client->get('UsersGroup?EmailUserID=' . $dccid);
    if($response->getStatusCode() != 200) {
      throw new RuntimeException(_txt('er.resttestdccprovisioner.usersgroup.query.failed') . ' (DccDocDbID = ' . $dccid . ')' . $full_name);
    }
    $users_group = array();
    $users_group_data = json_decode($response->getBody()->getContents(), true);
    foreach($users_group_data as $s) {
    ...
    foreach(array_keys($users_group) as $u){
      if(!array_key_exists($u, $co_groups_rev)){
        $response = $client->delete('UsersGroup/' . $users_group[$u]);
        if($response->getStatusCode() != 200){
          throw new RuntimeException(_txt('er.resttestdccprovisioner.usersgroup.delete.failed') . ' (DccDocDbID = ' . $dccid . ')' . $full_name);
        } else {
          $this->CoProvisioningTarget->Co->CoPerson->HistoryRecord->record($coPersonId,
                                                                           null,
                                                                           null,
                                                                           ...
                                                                           CakeSession::read('Auth.User.co_person_id'),
                                                                           ActionEnum::ProvisionerAction,
                                                                           _txt('pl.resttestdccprovisioner.removed', array($u)));
        } 
      }
    }
    ...
    foreach(array_keys($co_groups) as $g){
      if(!array_key_exists($co_groups[$g], $users_group)){
        ...
        $response = $client->post('UsersGroup', [GuzzleHttp\RequestOptions::JSON => $usersgroup_data]);
        if($response->getStatusCode() != 201){
          throw new RuntimeException(_txt('er.resttestdccprovisioner.usersgroup.create.failed') . ' (DccDocDbID = ' . $dccid . ')' . $full_name);
        } else {
          // Success !
          $this->CoProvisioningTarget->Co->CoPerson->HistoryRecord->record($coPersonId,
                                                                           null,
                                                                           ...
                                                                           CakeSession::read('Auth.User.co_person_id'),
                                                                           ActionEnum::ProvisionerAction,
                                                                           _txt('pl.resttestdccprovisioner.added', array($g)));
        }
      }
    }
